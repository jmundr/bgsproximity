...
const storeConfig: Store.Options<ISettings> = {
	migrations: {
		...
		'1.1.5': (store) => {
			...
			if (serverURL === 'http://54.193.94.35:9736') {
				store.set('serverURL', 'https://bgs-proximity.glitch.me/');
			}
		},
		'1.1.6': (store) => {
		...
		'1.2.0': (store) => {
			if (store.get('serverURL') !== 'https://bgs-proximity.glitch.me/') {
				store.set('serverURL', 'https://bgs-proximity.glitch.me/');
			}
			// @ts-ignore
	...
	schema: {
		...
		serverURL: {
			type: 'string',
			default: 'https://bgs-proximity.glitch.me/',
			format: 'uri',
		},
		pushToTalkShortcut: {
			type: 'string',
			...
			<Dialog fullScreen open={open} onClose={() => setOpen(false)}>
				...
				<DialogContent className={className}>
					...
					<Alert severity="error">
						Changing this could allow for mallicious activity, keeping it on the default URL is best.
					</Alert>
					<Button
						color="primary"
						variant="contained"
						onClick={() => {
							setOpen(false);
							setURLValid(true);
							onValidURL('https://bgs-proximity.glitch.me/');
						}}
					>
						Reset to default
					</Button>
